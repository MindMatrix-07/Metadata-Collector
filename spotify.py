...
class Spotify:
    def __init__(self, client_id=None, client_secret=None) -> None:
        ...
        if not (self.client_id and self.client_secret):
            try:
                ...
                logging.error(f"Failed to auth spotify: {e}")
        else:
            try:
                ...
                logging.error(f"Failed to auth spotify with provided keys: {e}")
    ...
    def get_tarck(self, link=None, track=None):
        if not self.sp: return {"error": "Spotify API Credentials missing. Please set them in /api."}
        if link is not None: track = self.get_spotify_id(link)
        return self.sp.track(track)

    def get_album_tarck(self, id: str):
        if not self.sp: return {"items": []}
    ...
    def get_isrc(self, link):
        if not self.sp:
            return ["Spotify API Credentials missing. Please set them in the /api section."]
        isrcs = []
        track = None
        ...
        if match:
            ...
            if not tracks.get("items"):
                return ["Album not found or Spotify keys are missing."]
